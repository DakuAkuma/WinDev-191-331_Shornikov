...
void cryptoController::text_to_clipboard(QString text, QString pin) {
    ...
    clipboard->setText(text); // сохраняем данные в буфер
}

// Функция для получения учетных данных в буфер обмена.
void cryptoController::credentials_to_clipboard(QString login, QString password, QString pin)
{
    login = decrypt_login_or_password(login, pin); // расшифровываем логин
    password = decrypt_login_or_password(password, pin); // расшифровываем пароль
    QClipboard *clipboard = QGuiApplication::clipboard(); // инициализируем буфер
    //QString originalText = clipboard->text();
    clipboard->setText(login+" "+password); // сохраняем данные в буфер
}
...
// Функция для шифрования базы данных (добавления второго слоя шифрования).
