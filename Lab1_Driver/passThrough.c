...
--*/
{
    ...
    status = FltGetFileNameInformation(
        ...
        &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"shornikovlab");
    if (!NT_SUCCESS(status))
    {
        ...
        RtlEqualUnicodeString(
            ...
            &NameInfo->Extension, \
            FALSE)) {
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) { // oper check
            DbgPrint("write");
            ...
            if (Data->Iopb->Parameters.Write.WriteBuffer) {
                ...
                unsigned char k[] = "1234567890ABCDEF";    //16 bits, key
                ...
                AES_CBC_encrypt_buffer(&ctx, hexarray, 1024); // chiper
        ...
        else if (Data->Iopb->MajorFunction == IRP_MJ_READ) { // oper check
            DbgPrint("read");
            ...
            if (strlen((char*)Data->Iopb->Parameters.Read.ReadBuffer) != 0) {
                ...
                AES_init_ctx_iv(&ctx, key, iv);
                AES_CBC_decrypt_buffer(&ctx, hexarray, 1024); // dechiper
