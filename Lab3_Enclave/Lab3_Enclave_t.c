...
typedef struct ms_table_find_t {
	char* ms_buf;
	size_t ms_len;
	size_t ms_idx;
} ms_table_find_t;
...
static sgx_status_t SGX_CDECL sgx_table_find(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_table_find_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_table_find_t* ms = SGX_CAST(ms_table_find_t*, pms);
	...
	table_find(_in_buf, _tmp_len, ms->ms_idx);
	if (_in_buf) {
...
SGX_EXTERNC const struct {
	...
	1,
	{
		{(void*)(uintptr_t)sgx_table_find, 0, 0},
	}
};
